...
class Spline():
    ...
    def bump_at(self, time, value, start=None, trail=None):
        if time > self._cps[-1].time:
            raise ValueError('time oob')
        if start:
            self.add_cp(CP(self.calculate()[:,time-start], time-start))
        if trail:
            self.add_cp(CP(self.calculate()[:,time+trail], time+trail))
        self.add_cp(CP(self.calculate()[:,time] + value, time))

    def add_cp(self, cp, index=-1, dt=50):
        #make sure time is higher than prior time
        if isinstance(cp, CP):
            i = 0
            while i < len(self._cps) and index==-1:
                if cp.time < self._cps[i].time:
                    index = i
                elif cp.time == self._cps[i].time:
                    self.del_cp(i)
                    index = i
                i += 1
        else:
            if not 0 <= index <= len(self._cps):
                raise ValueError('index ooo')
